...
class BLib {
	...
	#if h3d
	public inline function h_get(k:String, ?frame=0, ?xr=0., ?yr=0., ?filter:Null<Bool>, ?p:h2d.Object) : HSprite {
	...
	public inline function getColoredH2dBitmap(k:String, col:UInt, ?alpha=1.0, ?filter:Null<Bool>, ?parent:h2d.Object) {
		var e = getH2dBitmap(k, filter, parent);
		e.color = h3d.Vector.fromColor(mt.deepnight.Color.addAlphaF(col, alpha),1);
		return e;
	}

	public inline function getH2dBitmap(k:String, ?frame=0, ?xr=0.0, ?yr=0.0, ?filter:Null<Bool>, ?parent:h2d.Object, ?sh:h2d.Drawable.DrawableShader) : h2d.Bitmap {
		if( !exists(k,frame) )
			throw "Unknown group "+k+"#"+frame;
