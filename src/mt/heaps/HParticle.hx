...
class HParticle extends BatchElement {
	...
	public inline function setPosition(x:Float, y:Float) setPos(x,y);
		...
		if( delayF<=0 && !killed ) {
			...
			if( !killed ) {
				...
				if( !killed ) { // can be killed in onBounce
					...
					dsX    *= optimPow(dsFrict, dt);
					dsY    *= optimPow(dsFrict, dt);

					// Color animation
					if( toColor!=null ) {
						rColor = MLib.fclamp(rColor+dColor*dt, 0, 1);
