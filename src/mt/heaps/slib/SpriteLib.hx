...
class FrameData {
	...
	public var wid       : Int;
	public var hei       : Int;
	public var realFrame : RealFrameData;
	public var pX        : Null<Float>;
...
class SpriteLib {
	#if !macro
	public static var DT : Null<Float>;
	...
	******************************************************/
	...
	public function sliceCustom(groupName:String, page:Int, frame:Int, x:Int, y:Int, wid:Int, hei:Int, ?realFrame:RealFrameData, ?pX:Float,?pY:Float) {
		...
		g.frames[frame] = { page:page, x:x, y:y, wid:wid, hei:hei, realFrame:realFrame, pX:pX, pY:pY, tile: null };
	}
	...
	public function slice(groupName:String, page:Int, x:Int, y:Int, wid:Int, hei:Int, ?repeatX=1, ?repeatY=1) {
		...
		for(iy in 0...repeatY)
			for(ix in 0...repeatX)
				g.frames.push({page : page, x : x+ix*wid, y : y+iy*hei, wid:wid, hei:hei, realFrame:{x:0,y:0,realWid:wid,realHei:hei}, pX: null, pY: null, tile: null });
	}
	...
	public function sliceGrid(groupName:String, page:Int, gx:Int, gy:Int, ?repeatX=1, ?repeatY=1) {
		...
		for(iy in 0...repeatY)
			for(ix in 0...repeatX)
				g.frames.push({page : page, x : gridX*(gx+ix), y : gridY*(gy+iy), wid:gridX, hei:gridY, realFrame:{x:0,y:0,realWid:gridX,realHei:gridY}, pX: null, pY: null, tile: null });
	}
	...
	******************************************************/
	...
	public function updTile(t:h2d.Tile, g:String, ?frame=0, ?px:Float=0.0,?py:Float=0.0) : h2d.Tile {
		...
		t.dx = -Std.int( fd.realFrame.realWid * px + fd.realFrame.x );
		t.dy = -Std.int( fd.realFrame.realHei * py + fd.realFrame.y );
	...
	function __defineAnim(?group:String, anim:Array<Int>) {
		#if !macro
