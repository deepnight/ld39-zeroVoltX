...
class HSprite extends h2d.Drawable implements SpriteInterface {
	...
	inline function get_anim() {
		if( _animManager==null ) {
			_animManager = new mt.heaps.slib.AnimManager(this);
	...
	public inline function colorize(col:UInt, ?alpha=1.0) {
		color.setColor(mt.deepnight.Color.addAlphaF(col, alpha));
	}
	...
	override function sync(ctx:h2d.RenderContext) {
		super.sync(ctx);
		if( animAllocated )
			anim.update( SpriteLib.DT!=null ? SpriteLib.DT : ctx.elapsedTime*hxd.Timer.wantedFPS );
	}
}
