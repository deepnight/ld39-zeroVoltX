...
class Cooldown {
	...
	public function new(fps:Float) {
		if( INDEXES == null )
			INDEXES = [for (str in haxe.rtti.Meta.getType(mt.Cooldown).indexes) Std.string(str)];
		reset();
		baseFps = fps;
	}
	...
	static function getMeta() : MetaAccess {
		for( m in Context.getModule("mt.Cooldown") )
			switch( m ){
			case TInst(ct,_):
				var c = ct.get();
				if( c.name == "Cooldown" ){
		...
		Context.error("Unable to getModule mt.Cooldown", Context.currentPos());
		return null;
	}
	...
	static function key( k : ExprOf<String> ) : ExprOf<Int> {
		...
		return macro {
			...
			// var v = $subIdx;
			// if( v < 0 || v > 0x3FFFFF )
			// 	trace("Warning: out of bounds cooldown sub-index: \""+mt.Cooldown.INDEXES[$eIndex>>22]+"\" + "+v);
			// #end
